...
class BanyungongPipeline(object):
    def __init__(self):
    ...
    def _conditional_insert(self, tx, item):
        tx.execute("select * from navigation_banyungong  where link= %s",(item['link'],))
        result=tx.fetchone()
        ...
        # all this block run on it's own thread
            if item.get('link'):
                tx.execute(\
                    "insert into navigation_banyungong (title,type,link) values (%s,%s, %s)",
                    (item['title'],
                    item['type'],
                    item['link'])
                    )
